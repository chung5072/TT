  ...
  const divStatus = useSelector((state: RootState) => state.left.divStatus )
    return (
        <div className="game-play" id={divStatus === 1 ? 'player-profile-theme' : divStatus === 2 ? 'map-theme' : divStatus === 3 ? 'item-theme' : divStatus === 0 ? 'basic-theme':divStatus === 4 ? 'monster-theme' : 'rulebook-theme'}>
          {divStatus === 1 ? <PlayerProfile/> : divStatus === 2 ? <Map client = {client} gameId = {gameId}/> : divStatus === 3 ? <ItemList/> : divStatus === 0 ? <div/>: divStatus === 4 ? <MonsterList/> : divStatus === 5 ? <RuleBook/> : divStatus === 6 ? <MonsterStage monsterState = {monsterState}/>: <GmControl client = {client} gameId = {gameId}/>}
        </div>
